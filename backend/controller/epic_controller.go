...
func UpdateEpic() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		result := epicCollection.FindOneAndUpdate(ctx, bson.M{"_id": updateId}, update)
		if result.Err() != nil {
...
*/
func UpdateManyEpic() gin.HandlerFunc {
	return func(c *gin.Context) {
		ctx, cancel := context.WithTimeout(context.Background(), timeoutLimit)
		defer cancel()
		validate := validator.New()
...
// Delete only one epic base on id of that epic
func DeleteOneEpic() gin.HandlerFunc {
	return func(c *gin.Context) {
		ctx, cancel := context.WithTimeout(context.Background(), timeoutLimit)
		defer cancel()

		// Convert the hex string to ObjectID
		deleteId, convertErr := primitive.ObjectIDFromHex(c.Param("epic_id"))
		if convertErr != nil {
			c.JSON(http.StatusBadRequest, "Invalid project ID: "+convertErr.Error())
			return
		}
		...
		result, deleteErr := epicCollection.DeleteOne(ctx, bson.M{"_id": deleteId})
		if deleteErr != nil {
