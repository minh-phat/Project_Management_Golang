...
func GetAllTasksInProject() gin.HandlerFunc {
	return func(c *gin.Context) {
		ctx, cancel := context.WithTimeout(context.Background(), timeoutLimit)
		defer cancel()
		...
		projectID, convertErr := primitive.ObjectIDFromHex(c.Param("projectID"))
		if convertErr != nil {
		...
		pipeline := mongo.Pipeline{
			...
			bson.D{
				{Key: "$lookup", Value: bson.D{
					...
					{Key: "localField", Value: "epics._id"},
					{Key: "foreignField", Value: "epic"},
					{Key: "as", Value: "tasks"},
				}},
			},
			bson.D{
				{Key: "$unwind", Value: "$tasks"}, // Unwind the task array
			},
			bson.D{
				{Key: "$replaceRoot", Value: bson.M{"newRoot": "$tasks"}}, // Replace root with task
			},
...
func UpdateStatusOfTask() gin.HandlerFunc {
	return func(c *gin.Context) {
		ctx, cancel := context.WithTimeout(context.Background(), timeoutLimit)
		defer cancel()
		...
		// Convert the hex string to an ObjectID
		taskId, convertErr := primitive.ObjectIDFromHex(c.Param("taskID"))
		if convertErr != nil {
			c.JSON(http.StatusBadRequest, "Invalid task ID: "+convertErr.Error())
			return
		}
		...
		var TaskNotUpdate model.Task
		// Find and update the epic in DB
		errorUpdateTask := taskCollection.FindOneAndUpdate(ctx, bson.M{"_id": taskId}, update).Decode(&TaskNotUpdate)
		if errorUpdateTask != nil {
			c.JSON(http.StatusInternalServerError, "Error updating epic: "+errorUpdateTask.Error())
			return
		}
		////////////////////////////////////////////////////////////////////////////////////////////////////
		/*
			Get task just updated
		*/
		// fmt.Println("taskID:", TaskNotUpdate.Id)

		// Define pipeline to filter the data by ID and join collections
		pipeline := mongo.Pipeline{
			bson.D{
				{Key: "$match", Value: bson.D{
					{Key: "_id", Value: TaskNotUpdate.Id},
				}},
			},
		}

		// Use the defined stages to aggregate data from the Epics and Employee collections
		ResultTaskJustUpdated, aggregateErr := taskCollection.Aggregate(ctx, pipeline)
		if aggregateErr != nil {
			c.JSON(http.StatusInternalServerError, "Error aggregating task: "+aggregateErr.Error())
			return
		}
		var TaskJustUpdated []gin.H

		// Decode the data from DB to the task array
		decodeErr := ResultTaskJustUpdated.All(ctx, &TaskJustUpdated)
		if decodeErr != nil {
			c.JSON(http.StatusInternalServerError, "Error decoding task: "+decodeErr.Error())
			return
		}
		...
		c.JSON(http.StatusOK, gin.H{
			"msg":         "1 task updated",
			"taskID":      TaskNotUpdate.Id,
			"taskUpdated": TaskJustUpdated[0],
		})
	}
}
