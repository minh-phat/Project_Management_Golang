<script>
	...
	import AdminSidebar from '$lib/components/Utilities/Sidebar/AdminSidebar.svelte';
	import EmployeeSidebar from '$lib/components/Utilities/Sidebar/AdminSidebar.svelte';
	import ManagerSidebar from '$lib/components/Utilities/Sidebar/AdminSidebar.svelte';
...
<div class="page-container flex h-auto">
	<div class="z-10 w-16 sm:w-auto">
		...
		<details class="dropdown-bottom z-10" open={openToggle}>
			...
			{#if currentUser.authorization == "admin"}
				<AdminSidebar />
			{:else if currentUser.authorization == "employee"}
				<EmployeeSidebar />
			{:else if currentUser.authorization == "manager"}
				<ManagerSidebar/>
			{:else}
				<EmployeeSidebar/>
			{/if}
	...
	<div class="content-container w-full">
		<Navbar headerName="Admin Dashboard" 
				headerSrc="Admin" 
