<script>
	...
	var authorization = field("authorization", "", [required()]);
	var description = field("description", "");
	const authorizationForm = form(authorization, description);

	async function HandleSubmit(event) {
		event.preventDefault();

		try {
			const response = await axios.post("/authorization-add", {
...
<section class="bg-transparent dark:bg-gray-900">
	<div class="py-2 px-4 mx-auto max-w-4xl lg:py-8">
		<h2 class="text-xl text-center font-bold text-gray-900 dark:text-white">Add authorization</h2>
